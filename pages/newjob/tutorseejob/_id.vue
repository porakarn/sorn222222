...
<p style="
  ...
  text-align: left;
  margin-bottom: 0px;
  color: #2c2929;">{{job.subject+ ' '+ job.subjectDetail + ' ' + job.extra}}    <strong v-if="status= 'ว่าง'" style=" background-color:#EEC0BB;
    border-radius: 6px;
    color: white;
    font-weight: 500;
    padding-right: 7px;
...
" v-if="job.day">
{{job.day.toString()}} {{job.time}} {{job.duration}} </p>
<p style="color:#545454; font-size:16px ; margin-bottom: 0px;" >
{{job.location}} {{job.province}}  </p> 

<p style="color:#545454; font-size:16px ; margin-bottom: 0px;" >
{{job.creator_gender}} {{job.creator_grade}}  </p> 

<p style="color:#545454; font-size:16px ; margin-bottom: 0px;" >
จุดประสงค์ {{job.purpose}}  </p> 
...
<br>
<div style="text-align:center;">
  <b-button @click="interest"  size="lg" style=" color:white;  background-color: #DFAEA9; border: 0px; 
    width:60%;">สนใจงานนี้</b-button>
